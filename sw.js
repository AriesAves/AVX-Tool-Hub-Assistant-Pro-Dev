// AVX Tool HUB Assistant PRO — Service Worker
...
const SHELL_FILES = [
  ...
  '/AVX-Tool-Hub-Assistant-Pro/index.html',
  '/AVX-Tool-Hub-Assistant-Pro/manifest.json'
];
  ...
  event.respondWith(
    fetch(request)
      .then(response => {
        ...
        if (response.ok && SHELL_FILES.some(f => request.url.endsWith(f.replace('/AVX-Tool-Hub-Assistant-Pro', '')))) {
      ...
      .catch(() => {
        ...
        return caches.match(request).then(cached => {
          ...
          if (request.mode === 'navigate') {
            return caches.match('/AVX-Tool-Hub-Assistant-Pro/index.html');
          }
        });
      })
  );
...
self.addEventListener('push', event => {
  ...
  self.registration.showNotification(data.title || 'AVX Tool HUB', {
    body: data.body || '',
    icon: '/AVX-Tool-Hub-Assistant-Pro/icons/icon-192.png',
    badge: '/AVX-Tool-Hub-Assistant-Pro/icons/icon-192.png'
  });
});
